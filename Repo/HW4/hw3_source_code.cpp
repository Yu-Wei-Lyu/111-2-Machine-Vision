...
public:
	...
	void updateQuadTreeRecursively(int treeHeight, const Point2i pointBegin, const Point2i pointEnd) {
		...
		uchar mergeColor = this->getMergeColor(pixels);
		if (mergeColor == COLOR_BLACK || mergeColor == COLOR_WHITE || treeHeight == 0) {
			this->updateQuadTreeImage(pointBegin, pointEnd, mergeColor);
		}
		if (mergeColor == COLOR_GRAY && treeHeight != 0) {
...
int main() {
	...
	for (ImageInfo& imageInfo : imageInfoList) {
		...
		int layerTotal = log2(max(image.cols, image.rows));
		for (int i = 1; i <= layerTotal; i++) {
			quadTreeImage.createQuadTreeImage(i);
